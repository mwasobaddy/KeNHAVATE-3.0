...
class IdeaController extends Controller
{
    ...
    public function index(Request $request): Response
    {
        $filters = $request->only([
            ...
            'sort_by', 'sort_direction', 'tab',
        ]);

        // Default to personal tab if not specified
        $tab = $filters['tab'] ?? 'personal';

        // Set author_id filter for personal tab
        if ($tab === 'personal') {
        ...
        // Add permissions to each idea
        $user = $request->user();
        $ideasData = $ideas->toArray();
        $ideasData['data'] = collect($ideasData['data'])->map(function ($idea) use ($user) {
            $ideaModel = Idea::find($idea['id']); // Get the full model for permission checks

            return array_merge($idea, [
                'can_edit' => $ideaModel->canBeEditedBy($user),
                'can_submit' => $ideaModel->isDraft() && $ideaModel->author_id === $user->id,
            ]);
        })->toArray();
